...
def fmac_pic(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    ...
    def _get_batch_key_ranges(
        ...
        """Gets FMAC_PIC_KEY ranges from source table divided into batches."""
        start_date_str, end_date_str = __get_partition_range(partition_key)
        range_query = f"""
            WITH _ (FMAC_PIC_KEY, ROW_NBR) AS (
                SELECT FMAC_PIC_KEY
                      ,ROW_NUMBER() OVER(ORDER BY FMAC_PIC_KEY)
                FROM irb.FMAC_PIC
            ...
            SELECT ((ROW_NBR-1)/{batch_size}) AS row_nbr
                  ,MIN(FMAC_PIC_KEY) AS min_key
                  ,MAX(FMAC_PIC_KEY) AS max_key
            FROM _
    ...
    def _select_batch_data(sql_server, min_key: int, max_key: int) -> pl.DataFrame:
        """Fetch data from FMAC_PIC for a given FMAC_PIC_KEY range [min_key, max_key]."""
        query = f"""
            SELECT FMAC_PIC_KEY
                  ,SNPSHT_RVW_KEY
                  ,CUST_KEY
                  ...
                  ,UPDATED_DTTM
            FROM irb.FMAC_PIC
            WHERE FMAC_PIC_KEY BETWEEN {min_key} AND {max_key}
            """
    ...
    def _bin_records(
        ...
        source_ids = set(source_df["FMAC_PIC_KEY"].to_list())
        target_ids = set(target_df["FMAC_PIC_KEY"].to_list())
        ...
        if potential_update_ids:
            update_source_df = source_df.filter(
                pl.col("FMAC_PIC_KEY").is_in(potential_update_ids)
            )
            ...
            diff_condition = (
                ...
                | __values_differ("CUST_KEY", "CUST_KEY_target")
                | __values_differ("PIC_KEY", "PIC_KEY_target")
                | __values_differ("FMAC_PIC_STTS", "FMAC_PIC_STTS_target")
                | __values_differ("FMAC_PIC_NOTE", "FMAC_PIC_NOTE_target")
            ...
            diff_df = update_source_df.join(
                update_target_df, on="FMAC_PIC_KEY", how="left", suffix="_target"
            ).filter(diff_condition)

            update_ids = diff_df["FMAC_PIC_KEY"].to_list()
    ...
    def _insert_batch_data(source_df: pl.DataFrame, insert_ids: list) -> int:
        ...
        insert_data_df = source_df.filter(pl.col("FMAC_PIC_KEY").is_in(insert_ids))
        ...
        with sql_server_target() as conn_target:
            with conn_target.cursor() as cursor_target:
                cursor_target.execute(
                    "ALTER TABLE irb.FMAC_PIC DISABLE TRIGGER ALL"
                )                
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.FMAC_PIC ON"
                )                
                cursor_target.fast_executemany = True
                cursor_target.executemany(
                    """
                    INSERT INTO irb.FMAC_PIC 
                      (
                        FMAC_PIC_KEY
                       ,SNPSHT_RVW_KEY
                       ,CUST_KEY
                ...
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.FMAC_PIC OFF"
                )                
                cursor_target.execute(
                    "ALTER TABLE irb.FMAC_PIC ENABLE TRIGGER ALL"
                )
                conn_target.commit()
    ...
    def _update_batch_data(source_df: pl.DataFrame, update_ids: list) -> int:
        ...
        update_data = source_df.filter(pl.col("FMAC_PIC_KEY").is_in(update_ids))
        update_data_df = update_data.select(
            [
                pl.col("SNPSHT_RVW_KEY"),
                pl.col("CUST_KEY"),
                pl.col("PIC_KEY"),
                pl.col("FMAC_PIC_STTS"),
                pl.col("FMAC_PIC_NOTE"),
                pl.col("CREATED_BY"),
                pl.col("CREATED_DTTM"),
                pl.col("UPDATED_BY"),
                pl.col("UPDATED_DTTM"),
    ...
    for batch in merged_ranges.iter_rows(named=True):
        ...
        if len(delete_ids) > 0:
            context.log.error(
                ...
                f"FMAC_PIC_KEY {batch_min_key:} to {batch_max_key:} => "
