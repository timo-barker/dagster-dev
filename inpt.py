...
def inpt(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    """Upserts records between source and target INPT tables."""
    ...
    def _get_batch_key_ranges(
        ...
        """Gets INPT_KEY ranges from source table divided into batches."""
        start_date_str, end_date_str = __get_partition_range(partition_key)
        range_query = f"""
            WITH _ (INPT_KEY, ROW_NBR) AS (
                SELECT INPT_KEY
                      ,ROW_NUMBER() OVER(ORDER BY INPT_KEY)
                FROM irb.INPT
            ...
            SELECT ((ROW_NBR-1)/{batch_size}) AS row_nbr
                  ,MIN(INPT_KEY) AS min_key
                  ,MAX(INPT_KEY) AS max_key
            FROM _
    ...
    def _select_batch_data(sql_server, min_key: int, max_key: int) -> pl.DataFrame:
        """Fetch data from INPT for a given INPT_KEY range [min_key, max_key]."""
        query = f"""
            SELECT INPT_KEY
                  ,PRFL_KEY
                  ,EXPRT_TYP
                  ...
                  ,UPDATED_DTTM
            FROM irb.INPT
            WHERE INPT_KEY BETWEEN {min_key} AND {max_key}
            """
    ...
    def _bin_records(
        ...
        source_ids = set(source_df["INPT_KEY"].to_list())
        target_ids = set(target_df["INPT_KEY"].to_list())
        ...
        if potential_update_ids:
            update_source_df = source_df.filter(
                pl.col("INPT_KEY").is_in(potential_update_ids)
            )
            ...
            diff_condition = (
                ...
                | __values_differ("INPT_NOTE", "INPT_NOTE_target")
            ...
            diff_df = update_source_df.join(
                update_target_df, on="INPT_KEY", how="left", suffix="_target"
            ).filter(diff_condition)

            update_ids = diff_df["INPT_KEY"].to_list()
    ...
    def _insert_batch_data(source_df: pl.DataFrame, insert_ids: list) -> int:
        ...
        insert_data_df = source_df.filter(pl.col("INPT_KEY").is_in(insert_ids))
        ...
        with sql_server_target() as conn_target:
            with conn_target.cursor() as cursor_target:
                cursor_target.execute(
                    "ALTER TABLE irb.INPT DISABLE TRIGGER ALL"
                )                
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.INPT ON"
                )                
                cursor_target.fast_executemany = True
                cursor_target.executemany(
                    """
                    INSERT INTO irb.INPT 
                      (
                        INPT_KEY
                       ,PRFL_KEY
                       ,EXPRT_TYP
                ...
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.INPT OFF"
                )                
                cursor_target.execute(
                    "ALTER TABLE irb.INPT ENABLE TRIGGER ALL"
                )
                conn_target.commit()
    ...
    def _update_batch_data(source_df: pl.DataFrame, update_ids: list) -> int:
        ...
        update_data = source_df.filter(pl.col("INPT_KEY").is_in(update_ids))
    ...
    for batch in merged_ranges.iter_rows(named=True):
        ...
        if len(delete_ids) > 0:
            context.log.error(
                ...
                f"INPT_KEY {batch_min_key:} to {batch_max_key:} => "
