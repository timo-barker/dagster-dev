...
def fld(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    ...
    def _get_batch_key_ranges(
        ...
        """Gets FLD_KEY ranges from source table divided into batches."""
        start_date_str, end_date_str = __get_partition_range(partition_key)
        range_query = f"""
            WITH _ (FLD_KEY, ROW_NBR) AS (
                SELECT FLD_KEY
                      ,ROW_NUMBER() OVER(ORDER BY FLD_KEY)
                FROM irb.FLD
            ...
            SELECT ((ROW_NBR-1)/{batch_size}) AS row_nbr
                  ,MIN(FLD_KEY) AS min_key
                  ,MAX(FLD_KEY) AS max_key
            FROM _
    ...
    def _select_batch_data(sql_server, min_key: int, max_key: int) -> pl.DataFrame:
        """Fetch data from FLD for a given FLD_KEY range [min_key, max_key]."""
        query = f"""
            SELECT FLD_KEY
                  ,FLD_NM
                  ,FLD_ORDR
                  ...
                  ,UPDATED_DTTM
            FROM irb.FLD
            WHERE FLD_KEY BETWEEN {min_key} AND {max_key}
            """
    ...
    def _bin_records(
        ...
        source_ids = set(source_df["FLD_KEY"].to_list())
        target_ids = set(target_df["FLD_KEY"].to_list())
        ...
        if potential_update_ids:
            update_source_df = source_df.filter(
                pl.col("FLD_KEY").is_in(potential_update_ids)
            )
            ...
            diff_condition = (
                ...
                | __values_differ("FLD_ORDR", "FLD_ORDR_target")
                | __values_differ("FLD_TYP", "FLD_TYP_target")
                | __values_differ("FLD_IN", "FLD_IN_target")
                | __values_differ("FLD_OUT", "FLD_OUT_target")
                | __values_differ("FLD_STTS", "FLD_STTS_target")
                | __values_differ("FLD_NOTE", "FLD_NOTE_target")
            ...
            diff_df = update_source_df.join(
                update_target_df, on="FLD_KEY", how="left", suffix="_target"
            ).filter(diff_condition)

            update_ids = diff_df["FLD_KEY"].to_list()
    ...
    def _insert_batch_data(source_df: pl.DataFrame, insert_ids: list) -> int:
        ...
        insert_data_df = source_df.filter(pl.col("FLD_KEY").is_in(insert_ids))
        ...
        with sql_server_target() as conn_target:
            with conn_target.cursor() as cursor_target:
                cursor_target.execute(
                    "ALTER TABLE irb.FLD DISABLE TRIGGER ALL"
                )                
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.FLD ON"
                )                
                ...
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.FLD OFF"
                )                
                cursor_target.execute(
                    "ALTER TABLE irb.FLD ENABLE TRIGGER ALL"
                )
                conn_target.commit()
    ...
    def _update_batch_data(source_df: pl.DataFrame, update_ids: list) -> int:
        ...
        update_data = source_df.filter(pl.col("FLD_KEY").is_in(update_ids))
    ...
    for batch in merged_ranges.iter_rows(named=True):
        ...
        if len(delete_ids) > 0:
            context.log.error(
                ...
                f"FLD_KEY {batch_min_key:} to {batch_max_key:} => "
