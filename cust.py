...
@dg.asset(
    ...
    deps=["disable_fks"],
    description="irb.CUST",
    group_name="wps_clnt_grt",
    key=dg.AssetKey(["localdb", "cust"]),
    kinds={"sql"},
    partitions_def=daily_partition,
    required_resource_keys={"sql_server_source", "sql_server_target"},
)
def cust(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    """Upserts records between source and target CUST tables."""
    ...
    def _get_batch_key_ranges(
        ...
        """Gets CUST_KEY ranges from source table divided into batches."""
        start_date_str, end_date_str = __get_partition_range(partition_key)
        range_query = f"""
            WITH _ (CUST_KEY, ROW_NBR) AS (
                SELECT CUST_KEY
                      ,ROW_NUMBER() OVER(ORDER BY CUST_KEY)
                FROM irb.CUST
            ...
            SELECT (ROW_NBR/{batch_size}) AS row_nbr
                  ,MIN(CUST_KEY) AS min_key
                  ,MAX(CUST_KEY) AS max_key
            FROM _
    ...
    def _select_batch_data(sql_server, min_key: int, max_key: int) -> pl.DataFrame:
        """Fetch data from CUST for a given CUST_KEY range [min_key, max_key]."""
        query = f"""
            SELECT CUST_KEY
                  ,FMAC_PIC_KEY_MSTR
                  ,CUST_AK
                  ...
                  ,UPDATED_DTTM
            FROM irb.CUST
            WHERE CUST_KEY BETWEEN {min_key} AND {max_key}
            """
    ...
    def _bin_records(
        ...
        source_ids = set(source_df["CUST_KEY"].to_list())
        target_ids = set(target_df["CUST_KEY"].to_list())
        ...
        if potential_update_ids:
            update_source_df = source_df.filter(
                pl.col("CUST_KEY").is_in(potential_update_ids)
            )
            ...
            diff_condition = (
                __values_differ("FMAC_PIC_KEY_MSTR", "FMAC_PIC_KEY_MSTR_target")
                | __values_differ("CUST_AK", "CUST_AK_target")
                | __values_differ("CUST_STTS", "CUST_STTS_target")
                | __values_differ("CUST_NOTE", "CUST_NOTE_target")
            ...
            diff_df = update_source_df.join(
                update_target_df, on="CUST_KEY", how="left", suffix="_target"
            ).filter(diff_condition)

            update_ids = diff_df["CUST_KEY"].to_list()
    ...
    def _insert_batch_data(source_df: pl.DataFrame, insert_ids: list) -> int:
        ...
        insert_data_df = source_df.filter(pl.col("CUST_KEY").is_in(insert_ids))
        ...
        with sql_server_target() as conn_target:
            with conn_target.cursor() as cursor_target:
                cursor_target.execute(
                    "ALTER TABLE irb.CUST DISABLE TRIGGER ALL"
                )                
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.CUST ON"
                )                
                cursor_target.fast_executemany = True
                cursor_target.executemany(
                    """
                    INSERT INTO irb.CUST 
                      (
                        CUST_KEY
                       ,FMAC_PIC_KEY_MSTR
                       ,CUST_AK
                ...
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.CUST OFF"
                )                
                cursor_target.execute(
                    "ALTER TABLE irb.CUST ENABLE TRIGGER ALL"
                )
                conn_target.commit()
    ...
    def _update_batch_data(source_df: pl.DataFrame, update_ids: list) -> int:
        ...
        update_data = source_df.filter(pl.col("CUST_KEY").is_in(update_ids))
        update_data_df = update_data.select(
            [
                pl.col("FMAC_PIC_KEY_MSTR"),
                pl.col("CUST_AK"),
                pl.col("CUST_STTS"),
                pl.col("CUST_NOTE"),
                pl.col("CREATED_BY"),
    ...
    for batch in merged_ranges.iter_rows(named=True):
        ...
        if len(delete_ids) > 0:
            context.log.error(
                ...
                f"CUST_KEY {batch_min_key:} to {batch_max_key:} => "
