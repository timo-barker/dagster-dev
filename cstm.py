...
@dg.asset(
    ...
    key=dg.AssetKey(["localdb", "cstm"]),
    kinds={"sql"},
    partitions_def=daily_partition,
    required_resource_keys={"sql_server_source", "sql_server_target"},
)
def cstm(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    """Upserts records between source and target CSTM tables."""
    ...
    def _get_batch_key_ranges(
        ...
        """Gets CSTM_KEY ranges from source table divided into batches."""
        start_date_str, end_date_str = __get_partition_range(partition_key)
        range_query = f"""
            WITH _ (CSTM_KEY, ROW_NBR) AS (
                SELECT CSTM_KEY
                      ,ROW_NUMBER() OVER(ORDER BY CSTM_KEY)
                FROM irb.CSTM
            ...
            SELECT ((ROW_NBR-1)/{batch_size}) AS row_nbr
                  ,MIN(CSTM_KEY) AS min_key
                  ,MAX(CSTM_KEY) AS max_key
            FROM _
    ...
    def _select_batch_data(sql_server, min_key: int, max_key: int) -> pl.DataFrame:
        """Fetch data from CSTM for a given CSTM_KEY range [min_key, max_key]."""
        query = f"""
            SELECT CSTM_KEY
                  ,PRFL_KEY
                  ,RPT_STRT_DT
                  ...
                  ,UPDATED_DTTM
            FROM irb.CSTM
            WHERE CSTM_KEY BETWEEN {min_key} AND {max_key}
            """
    ...
    def _bin_records(
        ...
        source_ids = set(source_df["CSTM_KEY"].to_list())
        target_ids = set(target_df["CSTM_KEY"].to_list())
        ...
        if potential_update_ids:
            update_source_df = source_df.filter(
                pl.col("CSTM_KEY").is_in(potential_update_ids)
            )
            ...
            diff_condition = (
                ...
                | __values_differ("CSTM_STTS", "CSTM_STTS_target")
                | __values_differ("CSTM_NOTE", "CSTM_NOTE_target")
            ...
            diff_df = update_source_df.join(
                update_target_df, on="CSTM_KEY", how="left", suffix="_target"
            ).filter(diff_condition)

            update_ids = diff_df["CSTM_KEY"].to_list()
    ...
    def _insert_batch_data(source_df: pl.DataFrame, insert_ids: list) -> int:
        ...
        insert_data_df = source_df.filter(pl.col("CSTM_KEY").is_in(insert_ids))
        ...
        with sql_server_target() as conn_target:
            with conn_target.cursor() as cursor_target:
                cursor_target.execute(
                    "ALTER TABLE irb.CSTM DISABLE TRIGGER ALL"
                )                
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.CSTM ON"
                )                
                cursor_target.fast_executemany = True
                cursor_target.executemany(
                    """
                    INSERT INTO irb.CSTM 
                      (
                        CSTM_KEY
                       ,PRFL_KEY
                       ,RPT_STRT_DT
                ...
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.CSTM OFF"
                )                
                cursor_target.execute(
                    "ALTER TABLE irb.CSTM ENABLE TRIGGER ALL"
                )
                conn_target.commit()
    ...
    def _update_batch_data(source_df: pl.DataFrame, update_ids: list) -> int:
        ...
        update_data = source_df.filter(pl.col("CSTM_KEY").is_in(update_ids))
    ...
    for batch in merged_ranges.iter_rows(named=True):
        ...
        if len(delete_ids) > 0:
            context.log.error(
                ...
                f"CSTM_KEY {batch_min_key:} to {batch_max_key:} => "
