...
@dg.asset(
    ...
    name="snpsht",
    description="irb.SNPSHT",
    kinds={"sql", "table"},
    group_name="wps_clnt_grt",
)
def snpsht(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    """Upserts records between source and target SNPSHT tables."""
    ...
    def _get_batch_key_ranges(
        ...
        """Gets SNPSHT_KEY ranges from source table divided into batches."""
        start_date_str, end_date_str = __get_partition_range(partition_key)
        range_query = f"""
            WITH _ (SNPSHT_KEY, ROW_NBR) AS (
                SELECT SNPSHT_KEY
                      ,ROW_NUMBER() OVER(ORDER BY SNPSHT_KEY)
                FROM irb.SNPSHT
            ...
            SELECT ((ROW_NBR-1)/{batch_size}) AS row_nbr
                  ,MIN(SNPSHT_KEY) AS min_key
                  ,MAX(SNPSHT_KEY) AS max_key
            FROM _
    ...
    def _select_batch_data(sql_server, min_key: int, max_key: int) -> pl.DataFrame:
        """Fetch data from SNPSHT for a given SNPSHT_KEY range [min_key, max_key]."""
        query = f"""
            SELECT SNPSHT_KEY
                  ,SNPSHT_DTTM
                  ,SNPSHT_STTS
                  ...
                  ,UPDATED_DTTM
            FROM irb.SNPSHT
            WHERE SNPSHT_KEY BETWEEN {min_key} AND {max_key}
            """
    ...
    def _bin_records(
        ...
        source_ids = set(source_df["SNPSHT_KEY"].to_list())
        target_ids = set(target_df["SNPSHT_KEY"].to_list())
        ...
        if potential_update_ids:
            update_source_df = source_df.filter(
                pl.col("SNPSHT_KEY").is_in(potential_update_ids)
            )
            ...
            diff_condition = (
                __values_differ("SNPSHT_DTTM", "SNPSHT_DTTM_target")
                | __values_differ("SNPSHT_STTS", "SNPSHT_STTS_target")
                | __values_differ("SNPSHT_NOTE", "SNPSHT_NOTE_target")
            ...
            diff_df = update_source_df.join(
                update_target_df, on="SNPSHT_KEY", how="left", suffix="_target"
            ).filter(diff_condition)

            update_ids = diff_df["SNPSHT_KEY"].to_list()
    ...
    def _insert_batch_data(source_df: pl.DataFrame, insert_ids: list) -> int:
        ...
        insert_data_df = source_df.filter(pl.col("SNPSHT_KEY").is_in(insert_ids))
        ...
        with sql_server_target() as conn_target:
            with conn_target.cursor() as cursor_target:
                cursor_target.execute(
                    "ALTER TABLE irb.SNPSHT DISABLE TRIGGER ALL"
                )                
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.SNPSHT ON"
                )                
                cursor_target.fast_executemany = True
                cursor_target.executemany(
                    """
                    INSERT INTO irb.SNPSHT 
                      (
                        SNPSHT_KEY
                       ,SNPSHT_DTTM
                       ,SNPSHT_STTS
                ...
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.SNPSHT OFF"
                )                
                cursor_target.execute(
                    "ALTER TABLE irb.SNPSHT ENABLE TRIGGER ALL"
                )
                conn_target.commit()
    ...
    def _update_batch_data(source_df: pl.DataFrame, update_ids: list) -> int:
        ...
        update_data = source_df.filter(pl.col("SNPSHT_KEY").is_in(update_ids))
    ...
    for batch in merged_ranges.iter_rows(named=True):
        ...
        if len(delete_ids) > 0:
            context.log.error(
                ...
                f"SNPSHT_KEY {batch_min_key:} to {batch_max_key:} => "
