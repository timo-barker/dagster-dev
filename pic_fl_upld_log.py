...
@dg.asset(
    ...
    group_name="wps_pic",
    key=dg.AssetKey(["localdb", "pic_fl_upld_log"]),
    kinds={"sqlserver"},
...
def pic_fl_upld_log(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    """SCD Type 1 merge between source and target PIC_FL_UPLD_LOG tables."""
    ...
    def _select_batch_data(sql_server, min_key: int, max_key: int) -> pl.DataFrame:
        """Fetch data from PIC_FL_UPLD_LOG for a given IDENT range [min_key, max_key]."""
        query = f"""
            SELECT IDENT
                  ,PIC_ID
                  ,UPLD_END
    ...
    def _bin_records(
        ...
        if potential_update_ids:
            ...
            diff_condition = (
                ...
                | __values_differ("UPLD_END", "UPLD_END_target")
        ...
        with sql_server_target() as conn_target:
            with conn_target.cursor() as cursor_target:
                cursor_target.execute(
                    "ALTER TABLE irb.PIC_FL_UPLD_LOG DISABLE TRIGGER ALL"
                )
                cursor_target.fast_executemany = True
                cursor_target.executemany(
                    """
                    INSERT INTO irb.PIC_FL_UPLD_LOG 
                      (
                        IDENT
                       ,PIC_ID
                       ,UPLD_END
                ...
                cursor_target.execute(
                    "ALTER TABLE irb.PIC_FL_UPLD_LOG ENABLE TRIGGER ALL"
                )
                conn_target.commit()
