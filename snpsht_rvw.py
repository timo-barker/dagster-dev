...
@dg.asset(
    ...
    key=dg.AssetKey(["localdb", "snpsht_rvw"]),
    kinds={"sql"},
...
def snpsht_rvw(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    ...
    def _get_batch_key_ranges(
        ...
        """Gets SNPSHT_RVW_KEY ranges from source table divided into batches."""
        start_date_str, end_date_str = __get_partition_range(partition_key)
        range_query = f"""
            WITH _ (SNPSHT_RVW_KEY, ROW_NBR) AS (
                SELECT SNPSHT_RVW_KEY
                      ,ROW_NUMBER() OVER(ORDER BY SNPSHT_RVW_KEY)
                FROM irb.SNPSHT_RVW
            ...
            SELECT ((ROW_NBR-1)/{batch_size}) AS row_nbr
                  ,MIN(SNPSHT_RVW_KEY) AS min_key
                  ,MAX(SNPSHT_RVW_KEY) AS max_key
            FROM _
    ...
    def _select_batch_data(sql_server, min_key: int, max_key: int) -> pl.DataFrame:
        """Fetch data from SNPSHT_RVW for a given SNPSHT_RVW_KEY range [min_key, max_key]."""
        query = f"""
            SELECT SNPSHT_RVW_KEY
                  ,SNPSHT_KEY
                  ,SNPSHT_KEY_PREV
                  ...
                  ,UPDATED_DTTM
            FROM irb.SNPSHT_RVW
            WHERE SNPSHT_RVW_KEY BETWEEN {min_key} AND {max_key}
            """
    ...
    def _bin_records(
        ...
        source_ids = set(source_df["SNPSHT_RVW_KEY"].to_list())
        target_ids = set(target_df["SNPSHT_RVW_KEY"].to_list())
        ...
        if potential_update_ids:
            update_source_df = source_df.filter(
                pl.col("SNPSHT_RVW_KEY").is_in(potential_update_ids)
            )
            ...
            diff_condition = (
                __values_differ("SNPSHT_KEY", "SNPSHT_KEY_target")
                | __values_differ("SNPSHT_KEY_PREV", "SNPSHT_KEY_PREV_target")
                | __values_differ("SNPSHT_RVW_STTS", "SNPSHT_RVW_STTS_target")
                | __values_differ("SNPSHT_RVW_NOTE", "SNPSHT_RVW_NOTE_target")
            ...
            diff_df = update_source_df.join(
                update_target_df, on="SNPSHT_RVW_KEY", how="left", suffix="_target"
            ).filter(diff_condition)

            update_ids = diff_df["SNPSHT_RVW_KEY"].to_list()
    ...
    def _insert_batch_data(source_df: pl.DataFrame, insert_ids: list) -> int:
        ...
        insert_data_df = source_df.filter(pl.col("SNPSHT_RVW_KEY").is_in(insert_ids))
        ...
        with sql_server_target() as conn_target:
            with conn_target.cursor() as cursor_target:
                cursor_target.execute(
                    "ALTER TABLE irb.SNPSHT_RVW DISABLE TRIGGER ALL"
                )                
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.SNPSHT_RVW ON"
                )                
                cursor_target.fast_executemany = True
                cursor_target.executemany(
                    """
                    INSERT INTO irb.SNPSHT_RVW 
                      (
                        SNPSHT_RVW_KEY
                       ,SNPSHT_KEY
                       ,SNPSHT_KEY_PREV
                ...
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.SNPSHT_RVW OFF"
                )                
                cursor_target.execute(
                    "ALTER TABLE irb.SNPSHT_RVW ENABLE TRIGGER ALL"
                )
                conn_target.commit()
    ...
    def _update_batch_data(source_df: pl.DataFrame, update_ids: list) -> int:
        ...
        update_data = source_df.filter(pl.col("SNPSHT_RVW_KEY").is_in(update_ids))
        update_data_df = update_data.select(
            [
                pl.col("SNPSHT_KEY"),
                pl.col("SNPSHT_KEY_PREV"),
                pl.col("SNPSHT_RVW_STTS"),
                pl.col("SNPSHT_RVW_NOTE"),
                pl.col("CREATED_BY"),
                pl.col("CREATED_DTTM"),
                pl.col("UPDATED_BY"),
                pl.col("UPDATED_DTTM"),
    ...
    for batch in merged_ranges.iter_rows(named=True):
        ...
        if len(delete_ids) > 0:
            context.log.error(
                ...
                f"SNPSHT_RVW_KEY {batch_min_key:} to {batch_max_key:} => "
