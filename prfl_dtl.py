...
@dg.asset(
    ...
    deps=["prfl", "rpt_rvw", "fmac_pic"],
    description="irb.PRFL_DTL",
    group_name="wps_clnt_grt",
    key=dg.AssetKey(["localdb", "prfl_dtl"]),
    kinds={"sql"},
    partitions_def=daily_partition,
    required_resource_keys={"sql_server_source", "sql_server_target"},
)
def prfl_dtl(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    """Upserts records between source and target PRFL_DTL tables."""
    ...
    def _get_batch_key_ranges(
        ...
        """Gets PRFL_DTL_KEY ranges from source table divided into batches."""
        start_date_str, end_date_str = __get_partition_range(partition_key)
        range_query = f"""
            WITH _ (PRFL_DTL_KEY, ROW_NBR) AS (
                SELECT PRFL_DTL_KEY
                      ,ROW_NUMBER() OVER(ORDER BY PRFL_DTL_KEY)
                FROM irb.PRFL_DTL
            ...
            SELECT (ROW_NBR/{batch_size}) AS row_nbr
                  ,MIN(PRFL_DTL_KEY) AS min_key
                  ,MAX(PRFL_DTL_KEY) AS max_key
            FROM _
    ...
    def _select_batch_data(sql_server, min_key: int, max_key: int) -> pl.DataFrame:
        """Fetch data from PRFL_DTL for a given PRFL_DTL_KEY range [min_key, max_key]."""
        query = f"""
            SELECT PRFL_DTL_KEY
                  ,PRFL_KEY
                  ,RVW_KEY
                  ...
                  ,UPDATED_DTTM
            FROM irb.PRFL_DTL
            WHERE PRFL_DTL_KEY BETWEEN {min_key} AND {max_key}
            """
    ...
    def _bin_records(
        ...
        source_ids = set(source_df["PRFL_DTL_KEY"].to_list())
        target_ids = set(target_df["PRFL_DTL_KEY"].to_list())
        ...
        if potential_update_ids:
            update_source_df = source_df.filter(
                pl.col("PRFL_DTL_KEY").is_in(potential_update_ids)
            )
            ...
            diff_condition = (
                ...
                | __values_differ("CNTRCT_YR", "CNTRCT_YR_target")
                | __values_differ("PRFL_DTL_STTS", "PRFL_DTL_STTS_target")
                | __values_differ("PRFL_DTL_NOTE", "PRFL_DTL_NOTE_target")
            ...
            diff_df = update_source_df.join(
                update_target_df, on="PRFL_DTL_KEY", how="left", suffix="_target"
            ).filter(diff_condition)

            update_ids = diff_df["PRFL_DTL_KEY"].to_list()
    ...
    def _insert_batch_data(source_df: pl.DataFrame, insert_ids: list) -> int:
        ...
        insert_data_df = source_df.filter(pl.col("PRFL_DTL_KEY").is_in(insert_ids))
        ...
        with sql_server_target() as conn_target:
            with conn_target.cursor() as cursor_target:
                cursor_target.execute(
                    "ALTER TABLE irb.PRFL_DTL DISABLE TRIGGER ALL"
                )                
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.PRFL_DTL ON"
                )                
                cursor_target.fast_executemany = True
                cursor_target.executemany(
                    """
                    INSERT INTO irb.PRFL_DTL 
                      (
                        PRFL_DTL_KEY
                       ,PRFL_KEY
                       ,RVW_KEY
                ...
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.PRFL_DTL OFF"
                )                
                cursor_target.execute(
                    "ALTER TABLE irb.PRFL_DTL ENABLE TRIGGER ALL"
                )
                conn_target.commit()
    ...
    def _update_batch_data(source_df: pl.DataFrame, update_ids: list) -> int:
        ...
        update_data = source_df.filter(pl.col("PRFL_DTL_KEY").is_in(update_ids))
        update_data_df = update_data.select(
            [
                ...
                pl.col("FMAC_PIC_KEY"),
                pl.col("CNTRCT_YR"),
                pl.col("PRFL_DTL_STTS"),
                pl.col("PRFL_DTL_NOTE"),
                pl.col("CREATED_BY"),
                pl.col("CREATED_DTTM"),
                pl.col("UPDATED_BY"),
                pl.col("UPDATED_DTTM"),
    ...
    for batch in merged_ranges.iter_rows(named=True):
        ...
        if len(delete_ids) > 0:
            context.log.error(
                ...
                f"PRFL_DTL_KEY {batch_min_key:} to {batch_max_key:} => "
