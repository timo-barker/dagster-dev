...
@dg.asset(
    ...
    key=dg.AssetKey(["localdb", "rpt"]),
    kinds={"sql"},
    partitions_def=daily_partition,
    required_resource_keys={"sql_server_source", "sql_server_target"},
)
def rpt(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    """Upserts records between source and target RPT tables."""
    ...
    def _get_batch_key_ranges(
        ...
        """Gets RPT_KEY ranges from source table divided into batches."""
        start_date_str, end_date_str = __get_partition_range(partition_key)
        range_query = f"""
            WITH _ (RPT_KEY, ROW_NBR) AS (
                SELECT RPT_KEY
                      ,ROW_NUMBER() OVER(ORDER BY RPT_KEY)
                FROM irb.RPT
            ...
            SELECT ((ROW_NBR-1)/{batch_size}) AS row_nbr
                  ,MIN(RPT_KEY) AS min_key
                  ,MAX(RPT_KEY) AS max_key
            FROM _
    ...
    def _select_batch_data(sql_server, min_key: int, max_key: int) -> pl.DataFrame:
        """Fetch data from RPT for a given RPT_KEY range [min_key, max_key]."""
        query = f"""
            SELECT RPT_KEY
                  ,RPT_STRT_DTTM
                  ,RPT_END_DTTM
                  ...
                  ,UPDATED_DTTM
            FROM irb.RPT
            WHERE RPT_KEY BETWEEN {min_key} AND {max_key}
            """
    ...
    def _bin_records(
        ...
        source_ids = set(source_df["RPT_KEY"].to_list())
        target_ids = set(target_df["RPT_KEY"].to_list())
        ...
        if potential_update_ids:
            update_source_df = source_df.filter(
                pl.col("RPT_KEY").is_in(potential_update_ids)
            )
            ...
            diff_condition = (
                __values_differ("RPT_STRT_DTTM", "RPT_STRT_DTTM_target")
                | __values_differ("RPT_END_DTTM", "RPT_END_DTTM_target")
                | __values_differ("RPT_CNT", "RPT_CNT_target")
                | __values_differ("RPT_STTS", "RPT_STTS_target")
                | __values_differ("RPT_NOTE", "RPT_NOTE_target")
            ...
            diff_df = update_source_df.join(
                update_target_df, on="RPT_KEY", how="left", suffix="_target"
            ).filter(diff_condition)

            update_ids = diff_df["RPT_KEY"].to_list()
    ...
    def _insert_batch_data(source_df: pl.DataFrame, insert_ids: list) -> int:
        ...
        insert_data_df = source_df.filter(pl.col("RPT_KEY").is_in(insert_ids))
        ...
        with sql_server_target() as conn_target:
            with conn_target.cursor() as cursor_target:
                cursor_target.execute(
                    "ALTER TABLE irb.RPT DISABLE TRIGGER ALL"
                )                
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.RPT ON"
                )                
                ...
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.RPT OFF"
                )                
                cursor_target.execute(
                    "ALTER TABLE irb.RPT ENABLE TRIGGER ALL"
                )
                conn_target.commit()
    ...
    def _update_batch_data(source_df: pl.DataFrame, update_ids: list) -> int:
        ...
        update_data = source_df.filter(pl.col("RPT_KEY").is_in(update_ids))
        update_data_df = update_data.select(
            [
                pl.col("RPT_STRT_DTTM"),
                pl.col("RPT_END_DTTM"),
                pl.col("RPT_CNT"),
                pl.col("RPT_STTS"),
                pl.col("RPT_NOTE"),
    ...
    for batch in merged_ranges.iter_rows(named=True):
        ...
        if len(delete_ids) > 0:
            context.log.error(
                ...
                f"RPT_KEY {batch_min_key:} to {batch_max_key:} => "
