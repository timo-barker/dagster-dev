...
@dg.asset(
    ...
    description="irb.PIC_TRX_LOG",
    group_name="wps_clnt_grt",
    key=dg.AssetKey(["localdb", "pic_trx_log"]),
    kinds={"sql"},
    partitions_def=daily_partition,
    required_resource_keys={"sql_server_source", "sql_server_target"},
)
def pic_trx_log(context: dg.AssetExecutionContext) -> dg.MaterializeResult:
    """Upserts records between source and target PIC_TRX_LOG tables."""
    ...
    def _get_batch_key_ranges(
        ...
        """Gets PIC_TRX_LOG_KEY ranges from source table divided into batches."""
        start_date_str, end_date_str = __get_partition_range(partition_key)
        range_query = f"""
            WITH _ (PIC_TRX_LOG_KEY, ROW_NBR) AS (
                SELECT PIC_TRX_LOG_KEY
                      ,ROW_NUMBER() OVER(ORDER BY PIC_TRX_LOG_KEY)
                FROM irb.PIC_TRX_LOG
            ...
            SELECT (ROW_NBR/{batch_size}) AS row_nbr
                  ,MIN(PIC_TRX_LOG_KEY) AS min_key
                  ,MAX(PIC_TRX_LOG_KEY) AS max_key
            FROM _
    ...
    def _select_batch_data(sql_server, min_key: int, max_key: int) -> pl.DataFrame:
        """Fetch data from PIC_TRX_LOG for a given PIC_TRX_LOG_KEY range [min_key, max_key]."""
        query = f"""
            SELECT PIC_TRX_LOG_KEY
                  ,PROC_NM
                  ,TASK_NM
                  ...
                  ,UPDATED_DTTM
            FROM irb.PIC_TRX_LOG
            WHERE PIC_TRX_LOG_KEY BETWEEN {min_key} AND {max_key}
            """
    ...
    def _bin_records(
        ...
        source_ids = set(source_df["PIC_TRX_LOG_KEY"].to_list())
        target_ids = set(target_df["PIC_TRX_LOG_KEY"].to_list())
        ...
        if potential_update_ids:
            update_source_df = source_df.filter(
                pl.col("PIC_TRX_LOG_KEY").is_in(potential_update_ids)
            )
            ...
            diff_condition = (
                __values_differ("PROC_NM", "PROC_NM_target")
                | __values_differ("TASK_NM", "TASK_NM_target")
                | __values_differ("SRC_NM", "SRC_NM_target")
                | __values_differ("TGT_NM", "TGT_NM_target")
                | __values_differ("STRT_DTTM", "STRT_DTTM_target")
                | __values_differ("END_DTTM", "END_DTTM_target")
                | __values_differ("ROW_CNT", "ROW_CNT_target")
                | __values_differ("PIC_TRX_LOG_STTS", "PIC_TRX_LOG_STTS_target")
                | __values_differ("PIC_TRX_LOG_NOTE", "PIC_TRX_LOG_NOTE_target")
            ...
            diff_df = update_source_df.join(
                update_target_df, on="PIC_TRX_LOG_KEY", how="left", suffix="_target"
            ).filter(diff_condition)

            update_ids = diff_df["PIC_TRX_LOG_KEY"].to_list()
    ...
    def _insert_batch_data(source_df: pl.DataFrame, insert_ids: list) -> int:
        ...
        insert_data_df = source_df.filter(pl.col("PIC_TRX_LOG_KEY").is_in(insert_ids))
        ...
        with sql_server_target() as conn_target:
            with conn_target.cursor() as cursor_target:
                cursor_target.execute(
                    "ALTER TABLE irb.PIC_TRX_LOG DISABLE TRIGGER ALL"
                )                
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.PIC_TRX_LOG ON"
                )                
                cursor_target.fast_executemany = True
                cursor_target.executemany(
                    """
                    INSERT INTO irb.PIC_TRX_LOG 
                      (
                        PIC_TRX_LOG_KEY
                       ,PROC_NM
                       ,TASK_NM
                ...
                cursor_target.execute(
                    "SET IDENTITY_INSERT irb.PIC_TRX_LOG OFF"
                )                
                cursor_target.execute(
                    "ALTER TABLE irb.PIC_TRX_LOG ENABLE TRIGGER ALL"
                )
                conn_target.commit()
    ...
    def _update_batch_data(source_df: pl.DataFrame, update_ids: list) -> int:
        ...
        update_data = source_df.filter(pl.col("PIC_TRX_LOG_KEY").is_in(update_ids))
        update_data_df = update_data.select(
            [
                ...
                pl.col("END_DTTM"),
                pl.col("ROW_CNT"),
                pl.col("PIC_TRX_LOG_STTS"),
                pl.col("PIC_TRX_LOG_NOTE"),
                pl.col("CREATED_BY"),
                pl.col("CREATED_DTTM"),
                pl.col("UPDATED_BY"),
                pl.col("UPDATED_DTTM"),
                pl.col("PIC_TRX_LOG_KEY"),
            ]
        )
    ...
    for batch in merged_ranges.iter_rows(named=True):
        ...
        if len(delete_ids) > 0:
            context.log.error(
                ...
                f"PIC_TRX_LOG_KEY {batch_min_key:} to {batch_max_key:} => "
